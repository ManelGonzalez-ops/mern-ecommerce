...
const listProducts = () => async (dispatch) => {
    try {
        ...
        const rawData = await fetch("https://localhost:8000/products");
...
const detailsProduct = (productId) => async (dispatch) => {
    try {
        ...
        const rawData = await fetch(`https://localhost:8000/products/${productId}`)
        const data = await rawData.json()
    ...
    try {
        ...
        if (product.delete) {
            ...
            if (product._id) {
                ...
                const rawData = await fetch("https://localhost:8000/products/update", {
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${userInfo.token}`
                    },
...
const addReview = (productId, review, rating, author) => async (dispatch) => {
    ...
    try {
        ...
        const rawData = await fetch("https://localhost:8000/products/review", {
            headers: {
                "Content-Type": "application/json"
            },
